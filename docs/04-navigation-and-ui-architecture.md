...
| `navController.popBackStack()`                                       | `router.back()` for ordinary navigation; `router.dismissTo('/(main)')` clears the alarm modal stack on dismissal.                                                                              |
...
- Task completion uses the shared `useAlarmDismissal()` hook to stop native sound, clear delivered notifications, clear the active firing snapshot, and call `router.dismissTo('/(main)')` (not `router.back()`). No native dismissed callback/event is required; the resulting cleared modal stack mirrors the original `AlarmViewModel.onAlarmDismissed()` clear-top behavior.
