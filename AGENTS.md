...
- **State:** Zustand (+ immer). One shared `alarmStore`, plus per-screen `homeStore` / `createAlarmStore` mirroring the Kotlin `ViewModel`/`StateFlow` contracts. A shared `settingsStore` persists `UserSettings` (one JSON row in the `settings` SQLite table), including `colorScheme`.
- **Theming:** light/dark mode via `src/theme/`. `colors.ts` holds `darkColors` (default) and `lightColors` (same `Colors` shape). `index.ts` exposes `useTheme()` (selects the active `Theme` from `settings.colorScheme`) and `createThemedStyles(factory)` (memoized `StyleSheet` from `theme.colors`). Screens/components must read colors through `useTheme()`/`createThemedStyles` — no hardcoded color literals. The root `src/app/_layout.tsx` applies the scheme platform-wide (`Appearance.setColorScheme` + `SystemUI.setBackgroundColorAsync`) and sets `StatusBar` style from the scheme.
...
- **Native module build:** Run `npm run build:native` (`scripts/build-native-module.js`) to compile the custom `alarm-scheduler` Expo module at `native/`. Accepts an optional platform argument (`android`, `ios`, or `all` — default `all`). The script first runs `expo prebuild --platform <p> --no-install` if the native project directories (`android/`, `ios/`) don't exist. For **Android**, it invokes `./gradlew :alarm-scheduler:assembleRelease` in the generated `android/` directory, compiling only the alarm-scheduler library (not the full app). It also generates `android/local.properties` (with the detected SDK path) only when the file is absent — an existing file is preserved by default; pass `--overwrite-local-properties` to regenerate it (e.g. after moving the Android SDK). For **iOS**, it runs `pod install` in the generated `ios/` directory to build the CocoaPods-based module. Run this after making changes to any file under `native/android/` (Kotlin sources, `build.gradle`, `AndroidManifest.xml`) or `native/ios/` (Swift sources, `AlarmScheduler.podspec`) to verify the module compiles before a full `expo run:android`/`expo run:ios`. The `native/` module is auto-linked via `expo.autolinking.nativeModulesDir: "./native"` in `package.json` and its config plugin (`native/app.plugin.js`) is registered in `app.json`. A second config plugin (`plugins/withCxxCleanFix.js`) patches the generated `android/app/build.gradle` so `./gradlew clean` doesn't break CMake on the new architecture (it deletes `.cxx` before `externalNativeBuildClean*` runs); it is also registered in `app.json` and applied by `expo prebuild`.
- **Alarm scheduling:** custom Expo native module for exact, wake-up, recurring alarms (iOS has no exact alarms — degraded UX is expected and must be documented). Android `AlarmNotificationManager` is the single owner of channel/foreground-notification construction; `expo-notifications` handles permission checks and iOS received/response listeners.
- **Sound:** native `AlarmSoundService` loops Android playback with `AudioAttributes.USAGE_ALARM` and keeps it alive as a foreground media-playback service. The shared native module exposes `playAlarmSound` / `stopAlarmSound`; iOS retains degraded foreground-only custom playback.
...
`Alarm` is the single persisted entity. See `docs/02` §1 and `docs/07` §5 for the full TypeScript interface. Key fields: `id` (auto-increment), `days: Weekday[]` (`[]` ⇒ every day at schedule time), `hour/minute` (24-hour internally), `task: AlarmTask` (`"Memory"|"Math"|"Shake phone"|"Random"|"None"`; `TaskType` is the concrete subset without `"Random"`), `rounds` 1–5, `difficulty: "Easy"|"Normal"|"Hard"`, `sound` (`"Default"` or sandbox `file://` URI), `snooze`, `enabled`. Weekday/task/difficulty constants live in `src/data/constants.ts` (`taskTypes` = concrete tasks, `alarmTasks` = picker order including `"Random"`).

**Random task:** `"Random"` is resolved to a concrete task (never `"None"`) in JS at fire time, once per trigger, inside `alarmFiringStore.setActive()` via `resolveActiveSnapshot()` (`src/alarms/activeSnapshot.ts`) and `resolveAlarmTask()` (`src/tasks/randomTask.ts`). The firing alarm is an `ActiveAlarmSnapshot` with a `resolvedTask`, which UI and routing must use. Scheduled payloads must keep the configured task (`"Random"`): strip `resolvedTask` with `toScheduledSnapshot()` before scheduling so every snooze or weekly trigger draws again. See `docs/05` §7.

`UserSettings` is the single persisted preferences object (one JSON row in the `settings` SQLite table, read/written via `src/data/userSettings.ts`, normalized by `src/settings/userSettings.ts`). Fields: `autoDismissEnabled`, `snoozeMinutes` (1–60, default 5), `showTileNumbers`, `language` (`"en"`|`"zh-Hant"`), `colorScheme` (`"dark"` default | `"light"`). Defaults live in `DEFAULT_USER_SETTINGS` (`src/data/constants.ts`). The `colorScheme` field drives the theme system (see Tech Stack → Theming).
...
8. **Shared completion handler.** Use one `useAlarmDismissal()` hook wrapping `stopAlarmSound()` + delivered-notification cleanup; every task calls it. Do not add a native dismissed callback/event.
9. **Alarm flow is modal.** The `(alarm)` group uses `presentation: 'fullScreenModal'`. Task completion calls `router.dismissTo('/(main)')` (not `router.back()`) to discard the alarm stack, mirroring the original `onAlarmDismissed` clear-top behavior.
